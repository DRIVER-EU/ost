...
public class EventDTO {
    ...
    public static class ListItem extends MinimalItem {
        ...
        public ListItem(Event event) {
            ...
            this.trialUserId = trialUserOptional.map(TrialUser::getId).orElse(0L);
            this.trialUserFirstName = trialUserOptional.map(t -> t.getAuthUser().getFirstName()).orElse("");
            this.trialUserLastName = trialUserOptional.map(t -> t.getAuthUser().getLastName()).orElse("");

            Optional<TrialRole> trialRoleOptional = Optional.ofNullable(event.getTrialRole());
            this.trialRoleId = trialRoleOptional.map(TrialRole::getId).orElse(0L);
            this.trialRoleName = trialRoleOptional.map(TrialRole::getName).orElse("");
        }
    }
}
